...
export function getConfig(env = process.env) {
  ...
  return {
    ...
    publishDateProperty: env.NOTION_PUBLISH_DATE_PROPERTY?.trim() || 'Publish Date',
...
export function pageToFrontmatter(page, config) {
  ...
  return {
    ...
    publishDate: propertyValue(props[config.publishDateProperty]),
    status,
    notionId: page.id,
    notionUrl: page.url || '',
...
export function renderMdx(frontmatter, blocks) {
  const metadata = [
    ...
    `publishDate: ${yamlString(frontmatter.publishDate)}`,
